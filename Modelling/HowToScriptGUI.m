...
name = sprintf('sin(t*2*pi*%.10f)',f(i));
...
a = sprintf('%.10f',7./f(i));
set(handles.axisEnd, 'String', a);
% This changes the step size

maxStep = 1/(200*f(27));
dt = min(1/(200*f(i)), maxStep);
b = sprintf('%.10f', dt);

set(handles.stepSize, 'String',b);
...
c= sprintf('BlackBoxFrequencyResponse%d',i);
set(handles.saveFile, 'String', c );
% Use the save button
